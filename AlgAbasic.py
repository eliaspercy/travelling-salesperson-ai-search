...
added_note = "This is a fairly standard A* Search algorithm, using the nearest neighbour algorithm, from the current " \
             "node to a goal node, to calculate the heuristic score, and the evaluation function f(z) simply " \
             "combines this with the path cost up until the current node."
...
class Node:
    def __init__(self, city: int or str, state: list, unvisited: list, path_cost: int or str) -> None:
        ...
        self.nn = None                  # Stored route for the completed nearest neighbour, obtained during h(z)
  
    def __lt__(self, other):            # For handling cases when the priority queue has two nodes with the same h_value
...
def a_star_search() -> (list, int):
    ...
    # Construct an "a priori" node; uses strings '-' for the current_city and path_cost as these don't exist.
    initial_node = Node('-', [], list(range(num_cities)), '-')

    frontier = PriorityQueue()          # The frontier consists of tuples containing the f_value for the given node beside the node itself.
    frontier.put((0, initial_node))     # The purpose of having tuples in this manner is to simply make it easier to sort the nodes, thus making everyone's life easier.
    ...
    while not frontier.empty():
        ...
        if current_node.goal():
            # print(f"Time take: {time.time() - START_TIME} seconds.")
            current_best = min(current_best, (current_node.g_value, current_node.state))
            # print(current_best)
            # return current_node.state, current_node.g_value
        ...
        for child in current_node.unvisited:
            if time.time() - START_TIME > TIME_LIMIT:
                ...
                return (current_node.f_value, current_node.nn) if current_best[0] == math.inf else current_best
...
tour_length, tour = a_star_search()
